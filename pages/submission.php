...
// Fetch data with status 'new' ordered by timestamp in descending order
$query = "SELECT * FROM userinput WHERE status = 'new' ORDER BY datetime DESC";
$result = $conn->query($query);
?>

<div class="main-content">
    <h2 id="Page-title" class="Page-title"><strong>New Submissions</strong></h2>
    <br>
    <a href="all_submission.php" class="button">View All Submissions</a>
    <br>
            ...
            <tbody>
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    <tr>
                        ...
                        <td class="whatsapp" data-whatsapp="<?php echo $row['whatsapp']; ?>"><a href="javascript:void(0);" onclick="copyToClipboard(this)"><?php echo $row['whatsapp']; ?></a></td>
...
<script>
    function updateStatus(select, id) {
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                // If status is changed from 'new' to 'pending', remove the row from the table
                if (status != 'new') {
                    var row = statusSymbol.parentElement.parentElement;
    ...
    function copyToClipboard(element) {
        var whatsapp = element.parentElement.getAttribute('data-whatsapp');
        var dummy = document.createElement("textarea");
        document.body.appendChild(dummy);
        dummy.value = whatsapp;
        dummy.select();
        document.execCommand("copy");
        document.body.removeChild(dummy);
        alert("WhatsApp number copied to clipboard: " + whatsapp);
    }
</script>
