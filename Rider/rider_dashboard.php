...
<div id="content">
    <div class="container table-container">
        ...
        <div class="tab-content" id="myTabContent">
            <!-- Today's Bookings Tab (allows status update) -->
            ...
            <!-- Future Bookings Tab (no status update) -->
            ...
            <!-- Completed Bookings Tab (no status update) -->
...
$(document).ready(function () {
    function fetchBookingsData() {
        $.ajax({
            ...
            success: function (data) {
                updateTable("#today", data.today, true); // allow status update
                updateTable("#future", data.future, false); // no status update
                updateTable("#completed", data.completed, false); // no status update
            },
            error: function () {
                ...
                if (allowUpdate) {
                    row += `<td>
                                <select class="status-dropdown" data-booking-id="${booking.booking_id}">
                                    ...
                                    <option value="done" ${
                                        booking.status === "done" ? "selected" : ""
                                    }>Done</option>
                                    <option value="cancelled" ${
                                        booking.status === "cancelled" ? "selected" : ""
                                    }>Cancelled</option>
                                </select>
                                <button class="btn-update-status btn btn-sm btn-primary" data-booking-id="${booking.booking_id}">Update</button>
                            </td>`;
                } else {
                    row += `<td>-</td>`; // Placeholder for "Update Status" column
                }
    ...
    $(document).on("click", ".btn-update-status", function () {
        ...
        // Disable the row (blur and disable)
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (data.status === "success") {
                    ...
                    // Re-enable the row if there's an error
