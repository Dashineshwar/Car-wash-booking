    ...
    <div class="container">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (empty($username_err) && empty($password_err)) {
                ...
                $sql = "SELECT rider_id, username, password, email, phone_no, vehicle_number, available, serving_postcode FROM rider WHERE username = ?";
                ...
                if ($stmt = $conn->prepare($sql)) {
                    ...
                    if ($stmt->execute()) {
                        ...
                        if ($stmt->num_rows == 1) {
                            ...
                            $stmt->bind_result($rider_id, $username, $hashed_password, $email, $phone_no, $vehicle_number, $available, $serving_postcode);
                            if ($stmt->fetch()) {
                                if (password_verify($password, $hashed_password)) {
                                    // Password is correct, so start a new session
                                    session_start();

                                    // Store rider data in session variables
                                    $_SESSION["loggedin"] = true;
                                    $_SESSION["riderId"] = $rider_id;
                                    $_SESSION["username"] = $username;
                                    $_SESSION["email"] = $email;
                                    $_SESSION["phone"] = $phone_no;
                                    $_SESSION["vehicle_number"] = $vehicle_number;
                                    $_SESSION["available"] = $available;
                                    $_SESSION["serving_postcode"] = $serving_postcode;

                                    // Redirect rider to dashboard
